...
#SBATCH -J probe_sweep                 # Name of the job
...
#SBATCH --output=logs/probe_sweep_%j.out
#SBATCH --error=logs/probe_sweep_%j.err
...
#! ############################################################
#! Sweep loop: for each config — launch, benchmark, kill
#! ############################################################
...
for CONFIG_NAME in "${SWEEP_CONFIGS[@]}"; do
    ...
    # Get the --dynamic-spec-config JSON for this config (prints "none" for vanilla)
    ...
    # Wait for server ready (up to SERVER_TIMEOUT seconds — model load can take 5-10 min)
